...
A personal collection of zsh/bash helper scripts shared across the user's machines. There is no build, test, or lint tooling — each script under `bin/` is a standalone executable. The repo's `.zshrc` is the canonical source of the shell function wrappers the user sources from their home `~/.zshrc`.

## Architecture

- `bin/<category>/*.sh` — scripts grouped by purpose. Categories: `git/`, `video/`, `files/`, `drives/`. Each script is self-contained and parses its own CLI flags.
- `bin/utils.sh` — shared library at the `bin/` root, sourced by every script in a subfolder via:
  ```bash
...
- **Source utils**: always source `utils.sh` via the `SCRIPT_DIR/../utils.sh` pattern above so the script works regardless of CWD.
...
Video scripts rely on `ffprobe`/`ffmpeg`, plus `bc` for arithmetic and `perl` for regex-heavy text munging (notably in `bin/video/rename-video-file.sh`). When adding scripts that need an external binary, follow the `require_binary` pattern in `bin/video/find-video-mkv-issues.sh`.
